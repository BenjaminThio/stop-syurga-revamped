...
@onready var villain_sprite: AnimatedSprite2D = villain.get_child(0)
@onready var villain_spare_particles: GPUParticles2D = villain.get_node("SpareParticles")
...
func select_option(option: int) -> void:
	...
	if option == OPTION.SPARE:
		if villain.spareable:
			...
			villain_sprite.rotation_degrees = 0
			villain_sprite.self_modulate.a /= 2
			villain_spare_particles.emitting = true
			owner.background_music_player.stop()
			Audio.play_sound("vaporized")
			await time.sleep(2, func() -> void: Transition.fade_in(canvas, func(): get_tree().change_scene_to_file("res://Scenes/main_menu.tscn")))
		else:
