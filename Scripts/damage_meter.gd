...
@export var front_meter_range: float = 50
@export var back_meter_range: float = 50
@export var max_damage: float = 1700
@export var min_damage: float = max_damage * 0.1
@export var pause_time: float = 1.7
...
func _process(_delta) -> void:
	if Input.is_action_just_pressed("accept") and tween != null and not has_damaged and not missed and slider != null:
		...
		var slay_animation: AnimatedSprite2D = weapon.get_node("AnimatedSprite2D")
