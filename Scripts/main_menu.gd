...
@onready var level_label: Label = $Info/Level
...
@onready var playtime: Label = $Info/Playtime
...
@onready var font = Global.get_font(default_font)
#@onready var quit_label: Label = $Options/Quit

func _ready():
	for category_node in [info, location, options] as Array[Node2D]:
...
func _process(_delta):
	if ModifiedInput.either_of_the_actions_just_pressed(["up", "down"]):
		...
		if selected_option == OPTION.CONTINUE:
			...
			if Global.has_encountered_villain:
